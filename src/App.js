...
const Content = () => {
  ...
  async function handleDownload(file) {
    ...
    try {
        ...
        const url = await Storage.get(file, {level: "protected"});
...
const handleListFiles = (e) => {
  e.preventDefault();
//function handleListFiles (event) {
 // async handleListFiles = (event) => {
//    event.preventDefault();
    ...
    Storage.list("", { level: "protected" })
      .then((result) => {
        if (result.length === 0) {
            console.log(`No Files uploaded by the user`);
        }
        else {
       ...
       console.log(result);
       console.log("display files--->", displayFiles[0]);
       return displayFiles;
       //console.log(result[0].eTag);
    /*    console.log(`Current user ${Auth.currentUserInfo}`);
    ...
    if (uploadList.length === 0) {
      ...
      for (i = 0; i < uploadList.length; i++) {
        ...
        uploadCompleted.push(
          Storage.put(fileList[id].name, fileList[id], {
  ...
  return (
    <SpaceBetween size="l">
      ...
      <Container
        ...
        header={
          <>
            <Header variant="h2">File List</Header>
            <Button variant="primary" onClick={handleListFiles}>
              View
            </Button>
            <React.Fragment>
              <Table celled={true}>
                ...
                {(displayFiles.map((each) => (

                //), v) =>
                <Table.Body>
                  <Table.Row >
                    <Table.Cell>{each.key}</Table.Cell>
                    <Table.Cell>{each.eTag}</Table.Cell>
                    <Table.Cell>{each.size}</Table.Cell>
                     <Table.Cell>
                      {<Button icon labelPosition="right" 
                       onClick={handleDownload.bind(this, each.key)}
                      ><Icon name="download" />
                      Download</Button>}
                    </Table.Cell>
                  </Table.Row>
